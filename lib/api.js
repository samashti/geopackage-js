...
import { connect as GeoPackageConnectionConnect } from './db/geoPackageConnection'
import BoundingBox from './boundingBox'
import GeometryData from './geom/geometryData'
import TileBoundingBoxUtils from './tiles/tileBoundingBoxUtils'
import FeatureTile from './tiles/features'
import { DataColumnsDao, DataColumns } from './dataColumns'
import DataTypes from './db/dataTypes'
import { GeometryColumns } from './features/columns'
 ...
 */
GeoPackageAPI.open = async function(gppathOrByteArray) {
  ...
  var connection = await GeoPackageConnectionConnect(gppathOrByteArray);

  if (gppathOrByteArray && typeof gppathOrByteArray === 'string') {
    var geoPackage = await new GeoPackage(path.basename(gppathOrByteArray), gppathOrByteArray, connection);
  } else {
    var geoPackage = await new GeoPackage('geopackage', undefined, connection);
  }
 ...
 * @return {Promise} promise that resolves with the open {@link module:geoPackage~GeoPackage} object or rejects with an  `Error`
 */
GeoPackageAPI.create = async function(gppath) {
  ...
  if (typeof(process) !== 'undefined' && process.version && process.version !== 'browser' && gppath) {
    try {
      fs.mkdirSync(path.dirname(gppath));
    } catch (e) { }
  }
  let connection = await GeoPackageConnectionConnect(gppath);
  connection.setApplicationId();
  let geopackage
  if (gppath) {
    geopackage = await new GeoPackage(path.basename(gppath), gppath, connection);
  } else {
    geopackage = await new GeoPackage('geopackage', undefined, connection);
  }
 ...
 */
GeoPackageAPI.createStandardWebMercatorTileTable = async function(geopackage, tableName, contentsBoundingBox, contentsSrsId, tileMatrixSetBoundingBox, tileMatrixSetSrsId, minZoom, maxZoom, tileSize) {
  tileSize = tileSize || 256;
  const tileMatrixSet = await geopackage.createTileTableWithTableName(tableName, contentsBoundingBox, contentsSrsId, tileMatrixSetBoundingBox, tileMatrixSetSrsId)
...
GeoPackageAPI.createFeatureTableWithDataColumnsAndBoundingBox = async function(geopackage, tableName, geometryColumn, featureColumns, dataColumns, boundingBox, boundingBoxSrsId) {
 ...
 */
GeoPackageAPI.queryForGeoJSONFeaturesInTableFromPath = async function(geoPackagePath, tableName, boundingBox) {
 ...
 * @param  {Function} doneCallback    Caled with err if one occurred
 */
GeoPackageAPI.iterateGeoJSONFeaturesFromPathInTableWithinBoundingBox = async function(geoPackagePath, tableName, boundingBox) {
  const geoPackage = await GeoPackageAPI.open(geoPackagePath)
  return geoPackage.iterateGeoJSONFeaturesInTableWithinBoundingBox(tableName, boundingBox);
}
 ...
 * @param  {Number}   north      EPSG:4326 northern boundary
 */
GeoPackageAPI.getGeoJSONFeaturesInTile = async function(geopackage, table, x, y, z, skipVerification) {
 ...
 * @return {VectorTile}
 */
GeoPackageAPI.getVectorTile = async function(geopackage, table, x, y, z) {
 ...
 * @return {Protobuf}
 */
GeoPackageAPI.getVectorTileProtobuf = async function(geopackage, table, x, y, z) {
 ...
 * @param  {Number}   north      EPSG:4326 northern boundary
 */
GeoPackageAPI.getFeaturesInBoundingBox = async function(geopackage, table, west, east, south, north) {
  await geopackage.indexFeatureTable(table)
  var featureDao = geopackage.getFeatureDao(table);
 ...
 * @return {Promise}
 */
GeoPackageAPI.getTileFromXYZ = function(geopackage, table, x, y, z, width, height, format) {
  x = Number(x);
  y = Number(y);
  z = Number(z);
  width = Number(width);
  height = Number(height);
  var tileDao = geopackage.getTileDao(table);
  var retriever = new GeoPackageTileRetriever(tileDao, width, height, format);
