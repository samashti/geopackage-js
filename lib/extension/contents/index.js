...
class ContentsIdExtension extends BaseExtension {
  ...
  async getOrCreateExtension() {
    await this.getOrCreate(ContentsIdExtension.EXTENSION_NAME, undefined, undefined, ContentsIdExtension.EXTENSION_CONTENTS_ID_DEFINITION, Extension.READ_WRITE)
    return await this.contentsIdDao.createTable()
  }
  has() {
  ...
  async createForTableName(tableName) {
  ...
  async getOrCreateIdForTableName(tableName) {
    const contentsId = await this.getOrCreateForTableName(tableName)
    return contentsId.id;
  }
  getOrCreateForContents(contents) {
    return this.getOrCreateForTableName(contents.table_name);
  }
  async getOrCreateForTableName(tableName) {
    var contentsId = this.getForTableName(tableName);
    if (contentsId) {
      return contentsId;
    }
    return await this.createForTableName(tableName);
  }
  ...
  async createIdsForType(type) {
    var tables = this.missingForType(type);
    for (const tableName of tables) {
      await this.getOrCreateForTableName(tableName)
    }
