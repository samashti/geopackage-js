...
class FeatureDao extends UserDao {
  constructor(geoPackage, table, geometryColumns, metadataDb) {
    ...
    if (!dao.getContents(geometryColumns)) {
      throw new Error('Contents table entry for ' + geometryColumns.table_name + ' does not exist');
    }
    if (!dao.getSrs(geometryColumns)) {
      throw new Error('Spatial Reference System for ' + geometryColumns.table_name + ' does not exist');
    }
      ...
      next: function () {
        ...
        if (!nextRow.done) {
          ...
          while (!nextRow.done && !geometry) {
            ...
            console.log('featureRow.values.property_1', featureRow.values.property_1)
...
function verifyFeature(geometry, boundingBox) {
  console.log('verify', geometry.type)
  try {
    if (geometry.type == 'Point') {
      return geometry;
    } else if (geometry.type == 'LineString') {
      var intersect = LineIntersect(geometry, boundingBox.toGeoJSON().geometry);
      console.log('intersect', intersect.features)
      if (intersect.features.length) {
        return geometry;
      }
      console.log('within', BooleanWithin(geometry, boundingBox.toGeoJSON().geometry))
      if (BooleanWithin(geometry, boundingBox.toGeoJSON().geometry)) {
        return geometry;
      }
