...
GeoPackageUtils.insertExtraFeatures = function(geopackage) {
  console.log('Inserting Extra Features And Indexing');
  var bitSystems = {
    "type": "Point",
    "coordinates": [
      -104.801918, 39.720014
  ...
  var poly2 = {
    geoJson: ngaVisitorCenterPolygon,
    name: 'NGA Visitor Center 2'
  };

  var featureDao
  featureDao = geopackage.getFeatureDao('point1');
  GeoPackageUtils.createFeature(geopackage, point1.geoJson, point1.name, featureDao);

  featureDao = geopackage.getFeatureDao('point2');
  GeoPackageUtils.createFeature(geopackage, point2.geoJson, point2.name, featureDao);

  featureDao = geopackage.getFeatureDao('line1');
  GeoPackageUtils.createFeature(geopackage, line1.geoJson, line1.name, featureDao);

  featureDao = geopackage.getFeatureDao('line2');
  GeoPackageUtils.createFeature(geopackage, line2.geoJson, line2.name, featureDao);

  featureDao = geopackage.getFeatureDao('polygon1');
  GeoPackageUtils.createFeature(geopackage, poly1.geoJson, poly1.name, featureDao);

  featureDao = geopackage.getFeatureDao('polygon2');
  GeoPackageUtils.createFeature(geopackage, poly2.geoJson, poly2.name, featureDao);
